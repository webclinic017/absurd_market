...
def get_data(ticker, start_date, last_trading_date):
    ...
    try:
        data = quandl.get( 'EOD/'+ticker,
                           start_date=start_date, end_date=last_trading_date,
                           api_key='-geeNg-ha48hadKiPVx4')
    except ValueError:
        return None
...
def subtract_delta_from_date(last_trading_date, delta):
    date = datetime.datetime.strptime(last_trading_date, '%Y-%m-%d')        
    lower_date = date - delta
    return lower_date.strftime('%Y-%m-%d')

    
def find_buy_or_sell(quandl_dict):
    overbought_tickers = []
    undersold_tickers = []
    # add the overbought and udersold tickers to the right list
    # return the two iists at the end
    # add a button to the gui to fid them
    # use two list boxes to show the results 
    delta = datetime.timedelta(days=30)
    for ticker, (name, last_trading_date) in quandl_dict.items():
        start_date = subtract_delta_from_date(last_trading_date, delta)
        ...
        if rsi_indicator[-1] >= 70:
            print(ticker + ' is overbought. Sell it!')
        elif rsi_indicator[-1] <= 30:
            print(ticker + ' is undersold. Buy it!')
        else:
            print('Nothing can be said for ' + ticker+ '.')
...
def build_plots(data):
    ...
    plt.subplots_adjust(left=.12, bottom=.24, right=.90, top=.9, wspace=.20, hspace=0)
...
def close_plot():
    """this function close the plot"""
    plt.close()


if __name__ == "__main__":
    """ This part is if this file runs by itself then only find_buy_or_sell function 
    will apply instead of plotting with GUI"""    
    import download_file_csv_dict
    quandl_dict = download_file_csv_dict.read_tickers_from_Quandl()
    find_buy_or_sell(quandl_dict)
