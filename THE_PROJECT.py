...
def get_data(quandl_code):
     """This function return the data from the Quandl platform"""
     
     data = quandl.get( quandl_code, start_date="2016-04-01", end_date="2019-05-01", api_key='-geeNg-ha48hadKiPVx4')
     #data = quandl.get( quandl_code, start_date="2000-01-01", end_date="2019-05-01", api_key='-geeNg-ha48hadKiPVx4')
...
"""
     ...
     ax.fill_between(data["Date"], roc_indicator, 10, where=(rsi_indicator>=10), facecolor='#8f2020', edgecolor='#8f2020', alpha=0.2)
     ax.fill_between(data["Date"], roc_indicator, -10, where=(rsi_indicator<=-10), facecolor='#386d13', edgecolor='#386d13', alpha=0.2)        
...
def plot_Doji(ax, data):
     """ This function signals Doji pattern.
############################## CDLDOJI - Doji ##################################
explanation:"""
     
     doji_pattern = talib.CDLDOJI(data['Open'], data['High'], data['Low'], data['Close'])
     #ax1.scatter(data['Date'][nonzero], data['Adj_High'][nonzero]+5, c='r', marker='o')
     #ax.plot(data["Date"], doji_pattern, label="Doji pattern", color="steelblue")
     
     return np.flatnonzero(doji_pattern)   

     
def plot_Hammer(ax, data):
...
explanation:"""
     ...
     ax.plot(data["Date"], hammer_pattern, label="Hammer pattern", color="steelblue")
     
     return np.flatnonzero(hammer_pattern)


def plot_Hanging(ax, data):
     """This function signals Hanging Man pattern.
######################## CDLHANGINGMAN - Hanging Man ###########################
explanation:"""

     hanging_pattern = talib.CDLHANGINGMAN(data['Open'], data['High'], data['Low'], data['Close'])
     
     ax.plot(data["Date"], hanging_pattern, label="Hanging Man pattern", color="steelblue")
     
     return np.flatnonzero(hanging_pattern)
     

def plot_Morning_Star(ax, data):
     """This function signals Morning Star pattern.
################### CDLMORNINGSTAR - Morning Star ##############################
explanation:"""

     morningstar_pattern = talib.CDLMORNINGSTAR(data['Open'], data['High'], data['Low'], data['Close'], penetration=0)
     
     ax.plot(data["Date"], morningstar_pattern, label="Morning Star pattern", color="steelblue")
     
     return np.flatnonzero(morningstar_pattern)
     

def plot_Evening_Star(ax, data):
     """This function signals Evening Star pattern.
###################### CDLEVENINGSTAR - Evening Star ###########################
explanation:"""

     eveningstar_pattern = talib.CDLEVENINGSTAR(data['Open'], data['High'], data['Low'], data['Close'], penetration=0)
     
     ax.plot(data["Date"], eveningstar_pattern, label="Evening Star pattern", color="steelblue")
     
     return np.flatnonzero(eveningstar_pattern)
     

def plot_Dark_Cloud_Cover(ax, data):
     """This function signals Dark Cloud Cover pattern.
################### CDLDARKCLOUDCOVER - Dark Cloud Cover #######################
explanation:"""

     darkcloud_pattern = talib.CDLDARKCLOUDCOVER(data['Open'], data['High'], data['Low'], data['Close'], penetration=0)
     
     ax.plot(data["Date"], darkcloud_pattern, label="Dark Cloud Cover pattern", color="steelblue")
     
     return np.flatnonzero(darkcloud_pattern)
     

def plot_Harami(ax, data):
     """This function signals Harami pattern.
####################### CDLHARAMI - Harami Pattern #############################
explanation:"""

     harami_pattern = talib.CDLHARAMI(data['Open'], data['High'], data['Low'], data['Close'])
     
     ax.plot(data["Date"], harami_pattern, label="Harami pattern", color="steelblue")
     
     return np.flatnonzero(harami_pattern)
     

def plot_Engulfing(ax, data):
...
explanation:"""
     ...
     ax.plot(data["Date"], engulfing_pattern, label="Engulfing pattern", color="steelblue")
     
     return np.flatnonzero(engulfing_pattern)
     

def plot_Piercing(ax, data):
     """This function signals Piercing pattern.
################### CDLPIERCING - Piercing Pattern #############################
explanation:"""

     piercing_pattern = talib.CDLPIERCING(data['Open'], data['High'], data['Low'], data['Close'])
     
     ax.plot(data["Date"], piercing_pattern, label="Piercing pattern", color="steelblue")
...
def build_subplot(shape, loc, rowspan, colspan, facecolor='#333333'):
     """This function set each subplot"""
     
     ax = plt.subplot2grid(shape, loc, rowspan=rowspan, colspan=colspan, facecolor=facecolor)
...
def price_annotation(ax, data):
     ...
     ax.annotate(str(data['Close'][-1]), (data['Date'][-1], data['Close'][-1]), xytext=(data['Date'][-1]+30, data['Close'][-1]), bbox=bbox_props)
...
def build_plots(data):
     ...
     fig = plt.figure(facecolor = '#3D3C3C')
     ax0 = build_subplot((39, 6), (0, 0), 6, 6, facecolor='#333333')
     ax1 = build_subplot((39, 6), (6, 0), 15, 6, facecolor='#333333')
     ax2 = build_subplot((39, 6), (21, 0), 6, 6, facecolor='#333333')
     ax3 = build_subplot((39, 6), (27, 0), 6, 6, facecolor='#333333')
     ax4 = build_subplot((39, 6), (33, 0), 6, 6, facecolor='#333333')
...
if __name__ == "__main__":
     data = get_data("EOD/AAPL")
     fig, ax0, ax1, ax2, ax3, ax4, axVol = build_plots(data)
     plot_standarddev(ax0, data)
     plot_candlestick(ax1, data)
     plot_ma(ax1, data)
     plot_volume(axVol, data)
     plot_rsi(ax2, data)
     plot_macd(ax3, data)
     plot_stoch(ax4, data)
     nonzero = plot_Doji(ax4, data)
     ax1.scatter(data['Date'][nonzero], data['Adj_High'][nonzero]+5, c='r', marker='o')
     
     #doji_pattern = talib.CDLDOJI(data['Open'], data['High'], data['Low'], data['Close'])
     #print(doji_pattern)
     #d = np.flatnonzero(doji_pattern)
     #ax1.scatter(data['Date'][d], data['Adj_High'][d]+5, c='r', marker='o')
     #print(doji_pattern[16])
     #print(type(doji_pattern))
